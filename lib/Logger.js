...
Logger.prototype.logRequestError = function (error, queuedRequest) {
  var errorMessage =
    ERROR_MESSAGE.replace('{0}', error.message)
                 .replace('{1}', (error.isUserActionable ? 'IS' : 'is NOT'))
                 .replace('{2}', queuedRequest.url);
  this.log(errorMessage, Logger.LOG_LEVEL_ERROR);
};

Logger.prototype.logRequestSuccess = function (queuedRequest) {
  var successMessage =
    SUCCESS_MESSAGE.replace('{0}', queuedRequest.url);
  this.log(successMessage, Logger.LOG_LEVEL_INFO);
};
