...
function stripApiKeyFromURL(url) {
  var matches = url.match(/(.*)\?apiKey=/);
...
RequestManager.prototype = {
  ...
  makeRequest: function(req) {
    ...
    var cached = cache.get(stripApiKeyFromURL(req.url));
    if(cached) {
  ...
  handleResponse: function(req, err, res, data) {
      ...
      if (error) {
        ...
        ttl = res.headers['cache-control'] ? res.headers['cache-control'].match(/max-age=(.*),/) : 0;
        ttl = ttl.length > 1 ? parseInt(ttl[1]) * 1000 : 0;
        cache.put(stripApiKeyFromURL(req.url), {error: error, item: item}, ttl);
