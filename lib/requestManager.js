...
exports.handleResponse = function (error, response, data, queuedRequest) {
  var logger = queuedRequest.logger,
    statusCode = (response && response.statusCode ? response.statusCode : null),
    item = (statusCode === 200 ? data : null); // Fall back to null item if not 200

  if (error) {
    logger.log(error, logger.LOG_LEVEL_ERROR);
  }
  if (statusCode) { // We may have an error but still get a response with a status
