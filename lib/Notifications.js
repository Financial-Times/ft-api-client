...
Notifications.prototype.fetchItems = function (passedConfig, aggregateResponse) {
  ...
  sinceDate = new Date(this.config.since);

  // Options for the node http request
  // TODO: This needs to be refactored immediately after the request implementation
  options = {
    host: this.config.apiDomain,
    path: this.pathMapper.getNotificationsPathUpToSince(this.config.limit, sinceDate)
      .slice('http://api.ft.com'.length), /* Take the http:// off :'D */
    method: 'GET'
  };
