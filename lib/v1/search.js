...
var searchBodyTemplateDefaults = {
    "queryString":"",
    "queryContext":{
        "curations":["ARTICLES"]
    },
...
module.exports = function (term, quantity) {
    ...
    //If term is an object, check if it walks and quacks like a searchBodyTemplate
    // If so, use it instead of default one;
    if(typeof term === 'object' && 'queryString' in term && 'queryContext' in term){
        searchBodyTemplate = term;
        usingDefaultTemplate = false;
    }else{
        searchBodyTemplate = searchBodyTemplateDefaults;
