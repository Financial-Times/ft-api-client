...
function onResponse (resolve, reject, err, response, body){
    ...
    result.articles = body.results[0].results.map( function (article) {
    ...
    if(body.results.length && body.results[0].facets){
        var allFacets = [];
        body.results[0].facets.forEach( function( facet){
            allFacets = allFacets.concat(facet.facetElements);
            result.meta.facets.push(new model.Facets(facet));
        });
    }

    // Add some stats
    try{
        result.meta.counts.total = body.results[0].indexCount;
        allFacets.sort(function(a, b){
            return b.count - a.count;
        });
        result.meta.facetElements = allFacets;
    }catch(e){
        // Oh well, we tried...
        console.error(e);
    }
