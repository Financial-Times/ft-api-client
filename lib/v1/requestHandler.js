...
module.exports = function (opts) {
    ...
    return new Promise (function (resolve, reject) {
        ...
        if (cachedItem) {
            self.emit('ft-api-client:v1:lru:cache_hit'); 
            debug('cache hit for %s', endpoint);
            cachedItem.cacheHit = true;
            return resolve(cachedItem);
        } else {
            self.emit('ft-api-client:v1:lru:cache_miss'); 
        }
        ...
        request({
                ...
                if (err) {
                    ...
                    self.emit('ft-api-client:v1:request.rejected'); 
                    return reject(err);
                }
                ...
                try {
                    ...
                    self.emit('ft-api-client:v1:request.resolved'); 
                    resolve(body);
                } catch (error) {
