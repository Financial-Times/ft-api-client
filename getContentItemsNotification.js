...
var GetChangesFromCapi = function (passedConfig) {
...
GetChangesFromCapi.prototype.config = {
	...
	errorDelay:				10000 // If there is an error from the CAPI how long the app will wait bfore retrying
	//apiKey: Must be passed
	//since: Must be passed
};

GetChangesFromCapi.prototype.fetchItems = function (passedConfig, aggregateResponse) {
	"use strict";
	var	options,
		request,
		self = this;
...
exports.GetChangesFromCapi = GetChangesFromCapi;
...
function handleRequestEnd (aggregateResponse, notifObj, textResponse) {
	...
	if (notifications.length === notifObj.config.limit) {
		...
		// We are finished
		notifObj.emit('loadComplete', aggregateResponse);
	}
}
